...
/// @param success Called when the task is finished.
/// @param failure Called when the task is interrupted by an error. TYFailureError is returned.
- (void)getPrivacyAuthorizationStatusSuccess:(void(^)(TuyaSmartMessageAuthorizationModel *model))success failure:(TYFailureError)failure;

/// Update status of privacy authorization.
/// @param model The model of a authorization request.
/// @param success Called when the task is finished.
/// @param failure Called when the task is interrupted by an error. TYFailureError is returned.
- (void)updatePrivacyAuthorizationStatusWithModel:(TuyaSmartMessageAuthorizationRequestModel *)model Success:(TYSuccessID)success failure:(TYFailureError)failure;
