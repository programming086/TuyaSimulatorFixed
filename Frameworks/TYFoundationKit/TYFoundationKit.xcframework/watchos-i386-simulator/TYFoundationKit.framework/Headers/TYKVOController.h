...
/**
 ...
 TYKVOKeyPath(string.length) => @"length"

 Or even the complex case:

 TYKVOKeyPath(string.lowercaseString.length) => @"lowercaseString.length".
 */
#define TYKVOKeyPath(KEYPATH) \
@(((void)(NO && ((void)KEYPATH, NO)), \
({ const char *tykvokeypath = strchr(#KEYPATH, '.'); NSCAssert(tykvokeypath, @"Provided key path is invalid."); tykvokeypath + 1; })))
...
/**
 ...
 TYKVOClassKeyPath(NSString, length) => @"length"
 TYKVOClassKeyPath(NSString, lowercaseString.length) => @"lowercaseString.length"
 */
#define TYKVOClassKeyPath(CLASS, KEYPATH) \
@(((void)(NO && ((void)((CLASS *)(nil)).KEYPATH, NO)), #KEYPATH))

NS_ASSUME_NONNULL_BEGIN

/**
 Key provided in the @c change dictionary of @c TYKVONotificationBlock that's value represents the key-path being observed
 */
extern NSString *const TYKVONotificationKeyPathKey;
...
/**
 ...
 @param change The change dictionary which also includes @c TYKVONotificationKeyPathKey
 */
typedef void (^TYKVONotificationBlock)(id _Nullable observer, id object, NSDictionary<NSKeyValueChangeKey, id> *change);

/**
 @abstract TYKVOController makes Key-Value Observing simpler and safer.
 @discussion TYKVOController adds support for handling key-value changes with blocks and custom actions, as well as the NSKeyValueObserving callback. Notification will never message a deallocated observer. Observer removal never throws exceptions, and observers are removed implicitly on controller deallocation. TYKVOController is also thread safe. When used in a concurrent environment, it protects observers from possible resurrection and avoids ensuing crash. By default, the controller maintains a strong reference to objects observed.
 */
@interface TYKVOController : NSObject
 ...
 */
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(TYKVONotificationBlock)block;
 ...
 */
- (void)observe:(nullable id)object keyPaths:(NSArray<NSString *> *)keyPaths options:(NSKeyValueObservingOptions)options block:(TYKVONotificationBlock)block;
