...
/**
 ...
 ThingKVOKeyPath(string.length) => @"length"

 Or even the complex case:

 ThingKVOKeyPath(string.lowercaseString.length) => @"lowercaseString.length".
 */
#define ThingKVOKeyPath(KEYPATH) \
@(((void)(NO && ((void)KEYPATH, NO)), \
({ const char *thingkvokeypath = strchr(#KEYPATH, '.'); NSCAssert(thingkvokeypath, @"Provided key path is invalid."); thingkvokeypath + 1; })))
...
/**
 ...
 ThingKVOClassKeyPath(NSString, length) => @"length"
 ThingKVOClassKeyPath(NSString, lowercaseString.length) => @"lowercaseString.length"
 */
#define ThingKVOClassKeyPath(CLASS, KEYPATH) \
@(((void)(NO && ((void)((CLASS *)(nil)).KEYPATH, NO)), #KEYPATH))

NS_ASSUME_NONNULL_BEGIN

/**
 Key provided in the @c change dictionary of @c ThingKVONotificationBlock that's value represents the key-path being observed
 */
extern NSString *const ThingKVONotificationKeyPathKey;
 ...
 */
- (void)observe:(nullable id)object keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(ThingKVONotificationBlock)block;
 ...
 */
- (void)observe:(nullable id)object keyPaths:(NSArray<NSString *> *)keyPaths options:(NSKeyValueObservingOptions)options block:(ThingKVONotificationBlock)block;
