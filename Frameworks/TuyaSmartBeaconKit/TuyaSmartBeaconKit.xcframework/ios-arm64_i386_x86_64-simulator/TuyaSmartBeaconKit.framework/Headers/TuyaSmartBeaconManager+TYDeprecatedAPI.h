//
//  TuyaSmartBeaconManager+TYDeprecatedAPI.h
//  TuyaSmartBeaconKit
//
//  Created by Tuya on 2021/11/23.
//
...
@interface TuyaSmartBeaconManager (TYDeprecatedAPI)
...
/// @param failure Called when the task is interrupted by an error.
- (void)activeBeacon:(TYBLEAdvModel *)advModel homeId:(long long)homeId success:(void (^)(TuyaSmartDeviceModel *deviceModel))success failure:(void(^)(NSError *error))failure __deprecated_msg("use `activateBeacon:homeId:success:failure:` instead");
